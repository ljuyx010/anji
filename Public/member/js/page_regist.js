$(document).ready(function(){
	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			company: {
				required: $.i18n.prop("请输入账号")
			},			
			password: {
				required: $.i18n.prop("请输入密码"),
				minlength: jQuery.format($.i18n.prop("密码长度不够")),
				maxlength: jQuery.format($.i18n.prop("密码长度过长"))
			},
			passwordAgain: {
				required: $.i18n.prop("请输入密码"),
				minlength: jQuery.format($.i18n.prop("密码长度不够")),
			...
			tel: {
				...
				digits: $.i18n.prop("请输入正确的电话")
			},
			qq: {
				required: $.i18n.prop("请输入QQ"),
				digits: $.i18n.prop("请输入正确的QQ")
			}
		}
	});
...
function regist(validate){	
	...
	if(validate.form()){
		if($("#checkBox").attr("checked")){
				...
				success: function(data){
					...
					if(data.hasOwnProperty("code")){
						if(data.code == 0){
							//注册成功
							alert('恭喜您注册成功！');
							location.href = "/index.php/Index/member";
						}else if(data.code == 1){
							//注册失败
							$(".login-error").html($.i18n.prop("注册失败"));
						}else if(data.code == 2){
							//公司已经被注册
							$("#company").addClass("error");
							$("#company").after(registError);						
							$("#company").next("label.repeated").text($.i18n.prop("帐号已被占用"));
							registError.show();
						}else{
							//系统错误
							$(".login-error").html($.i18n.prop("系统出错"));
						}
					}
				}
			});
			...
			//勾选隐私政策和服务条款
			$(".login-error").show();
			$(".login-error").html($.i18n.prop("您没有同意注册条款"));
		}
	}
}
