...
<div class="main">
   <div class="menu"><h3>在线订车</h3><p>Online ordering</p></div>
    <ul>
    <?php if(is_array($goods)): foreach($goods as $key=>$v): ?><li><a href="<?php echo U('/'.MODULE_NAME.'/details_'.$v['id']);?>"><div><img <?php if(!$v['pics']): ?>src="/Public/images/nopic.png"<?php elseif(strstr($v['pics'],'|')): ?>src="<?php echo substr($v['pics'],0,strpos($v['pics'], '|'));?>" <?php else: ?>src="<?php echo ($v["pics"]); ?>"<?php endif; ?>></div><p><?php echo ($v["title"]); ?></p>
    <span class="zjzc"><strong>车型:</strong><?php echo ($v["classname"]); ?></span>
