...
<div class="menu"><h3>资讯中心</h3><p>Information center</p></div>
    <ul class="news">
    <?php if(is_array($article)): foreach($article as $key=>$v): ?><li><span class="time"><?php echo (date('Y-m-d',$v["time"])); ?></span><a href="<?php echo U('/'.MODULE_NAME.'/ashow_'.$v['id']);?>" title="<?php echo ($v["title"]); ?>"><?php echo ($v["title"]); ?></a></li><?php endforeach; endif; ?>
	</ul>
    <div class="clear"></div>
...
<script>  
    ...
    wx.ready(function(){  
        var options = {  
            ...
            link: 'http://<?php echo ($_SERVER['SERVER_NAME']); ?>/index.php/Article/index.html', // 分享链接，记得使用绝对路径，不能用document.URL
