                    ...
                    <div class="project-list">
                        ...
                        <table class="table table-hover">
                            ...
                            <?php if(is_array($group)): foreach($group as $key=>$v): ?><tr>
                                ...
                                <a data-toggle="modal" data-target="#myModal" onclick="edit(<?php echo ($v["id"]); ?>)" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> 修改 </a>
                                <a href="<?php echo U('Admin/setaccess',array('id'=>$v['id']));?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> 分配权限</a>
                                <a href="<?php echo U('Admin/deleteg',array('id'=>$v['id']));?>" class="btn btn-warning btn-sm" onclick="return confirm('确定要删除吗？')"><i class="fa fa-trash"></i> 删除 </a>
                                </td>
...
<script type="text/javascript">
    function edit(id){
        ...
        $.ajax({
            ...
            url: "<?php echo U('Admin/updatag');?>" ,//url
            data: {id:id},
    ...
    function save (){
        $.ajax({
            ...
            type: "POST",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "<?php echo U('Admin/runaddg');?>" ,//url
            data: $('#qx').serialize(),
            success: function (result) {
                // console.log(result);//打印服务端返回的数据(调试用)
                if (result.cod == 200) {
