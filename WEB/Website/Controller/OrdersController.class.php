...
class OrdersController extends CommonController{
	...
	public function fpage (){
		...
		if($id){$where = array('uid'=>array('neq',''));}else{$where =array('uid'=>array('eq',''));} 		
	...
	public function runaddx(){
		...
		$orderSn = date('Ymd').substr(time(), -4);//生成订单号
		if($end-$start){$d=round(($end-$start)/3600/24);}else{$d=1;}
		if(I('money')){$money=I('money');}else{$money=(($gs['oilj']*$gs['oilh']+$gs['glf'])+$num*0.43)*$lc+$gs['lr']*$d;}
		$data = array(
			'ordernum' => $orderSn,
			'edr' => I('edr'),
			'money' => $money,
		...
		if($id){
			...
			$jg2=M('ordcar')->where('id='.$ko)->setField('carnum',$vo);
			}
		}else{
			$jg=$db->add($data);
