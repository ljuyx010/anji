...
class PayController extends Controller{
    ...
    public function doalipay(){
       ...
       $alipay_config=C('alipay_config');  
        ...
        $subject = $_POST['pname'];  //订单名称 //必填 通过支付页面的表单进行传递
        $total_fee = $_POST['sumprice']+$_POST['yf'];   //付款金额  //必填 通过支付页面的表单进行传递
        $body = $_POST['ordbody'];  //订单描述 通过支付页面的表单进行传递
        $show_url = $_POST['ordshow_url'];  //商品展示地址 通过支付页面的表单进行传递
        $anti_phishing_key = "";//防钓鱼时间戳 //若要使用请调用类文件submit中的query_timestamp函数
        $exter_invoke_ip = get_client_ip(); //客户端的IP地址
        
		$data=array('uname'=>$_POST['uname'],'tel'=>$_POST['tel'],'adress'=>$_POST['adress'],'remark'=>$_POST['remark'].' 运费:'.$_POST['yf']);
		M('orders')->where(array('order'=>$out_trade_no))->save($data);
        /************************************************************/
